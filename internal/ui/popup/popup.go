			...
			case tea.KeyMsg:
				...
				case tea.KeyEnter:
					...
					if value == "" && len(m.ListValues) > 2 {
						...
						if m.ListCursor >= len(m.ListValues) && m.ListCursor > 0 {
							m.ListCursor--
						}
						// ensure at least 2 lines remain
						for len(m.ListValues) < 2 {
	...
	case tea.KeyMsg:
		...
		case tea.KeyEnter:
			...
			if m.isBoolField(m.Cursor) {
				m.inBoolSelect = true
				if strings.ToLower(m.Fields[m.Cursor]) == "true" {
					m.boolIndex = 0
				} else {
					m.boolIndex = 1
				}
...
func validateInput(fieldName, value, rrType string) string {
	...
	case "content":
		...
		if value != "@" && !isIPv4(value) {
				return "Content must be a valid IPv4 address for A record"
			}
		case "AAAA":
		if value != "@" && !isIPv6(value) {
				return "Content must be a valid IPv6 address for AAAA record"
			}
		case "CNAME", "NS", "MX":
		if value != "@" && !isHostname(value) {
				return "Content must be a valid hostname"
			}
		default:
...
func (m *Model) viewBase() string {
	...
	if w := stringWidth(helpPlain); w > maxW {
		maxW = w
	}
	if maxW < 30 {
		maxW = 30
